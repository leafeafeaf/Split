...
export const frameSlice = createSlice({
  ...
  reducers: {
    ...
    // 프레임 데이터 설정을 위한 리듀서 추가
    setFrames: (state, action: PayloadAction<FrameData[]>) => {
      state.frames = action.payload
      state.progress = Math.min(action.payload.length * 10, 100)
    },
  },
  extraReducers: (builder) => {
    builder
...
export const { clearFrames, updateProgress, clearCurrentFrame, setFrames } = frameSlice.actions
export default frameSlice.reducer